...
class ReportController extends Controller
{
    ...
    public function paymentTypeSales(Request $request)
    {
        ...
        $paymentRows = collect([
            ...
            ['label' => 'Bank / Card', 'icon' => 'bi-credit-card', 'amount' => $cardAmount, 'orders_count' => $orders->filter(fn ($order) => (float) $order->paid_in_card > 0)->count(), 'percentage' => $totalCollected > 0 ? ($cardAmount / $totalCollected) * 100 : 0],
    ...
    public function exportPdf(Request $request)
    {
        ...
        if ($report === 'payment_type_sales') {
            ...
            $dataRows = collect([
                ...
                ['label' => 'Bank / Card', 'orders_count' => $orders->filter(fn ($o) => (float) $o->paid_in_card > 0)->count(), 'amount' => $cardAmount, 'percentage' => $totalCollected > 0 ? ($cardAmount / $totalCollected) * 100 : 0],
    ...
    public function exportCsv(Request $request)
    {
        ...
        $callback = function () use ($report, $startDate, $endDate, $paymentMethod, $filterType, $year, $request) {
            ...
            if ($report === 'payment_type_sales') {
                ...
                $rows = [['Cash', $orders->filter(fn ($o) => (float) $o->paid_in_cash > 0)->count(), $cashAmount], ['Bank / Card', $orders->filter(fn ($o) => (float) $o->paid_in_card > 0)->count(), $cardAmount], ['Mobile Banking / MFS', $orders->filter(fn ($o) => (float) $o->paid_in_mfc > 0)->count(), $mfcAmount]];
                foreach ($rows as $row) {
