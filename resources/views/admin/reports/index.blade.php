...
<a href="{{ route('reports.sales_order') }}" class="progga-btn progga-btn-outline progga-btn-sm js-report-reset"><i class="bi bi-x-circle"></i> Reset</a></form></div>
  <div class="row g-3 mb-4"><div class="col-md-3"><div class="progga-stat-card"><div class="progga-stat-icon secondary"><i class="bi bi-currency-dollar"></i></div><div class="progga-stat-info"><div class="progga-stat-label">Total Revenue</div><div class="progga-stat-value">৳{{ number_format($totalRevenue, 0) }}</div></div></div></div><div class="col-md-3"><div class="progga-stat-card"><div class="progga-stat-icon primary"><i class="bi bi-receipt"></i></div><div class="progga-stat-info"><div class="progga-stat-label">Total Orders</div><div class="progga-stat-value">{{ $totalOrders }}</div></div></div></div><div class="col-md-3"><div class="progga-stat-card"><div class="progga-stat-icon success"><i class="bi bi-graph-up"></i></div><div class="progga-stat-info"><div class="progga-stat-label">Avg. Order Value</div><div class="progga-stat-value">৳{{ number_format($avgOrderValue, 0) }}</div></div></div></div><div class="col-md-3"><div class="progga-stat-card"><div class="progga-stat-icon warning"><i class="bi bi-people"></i></div><div class="progga-stat-info"><div class="progga-stat-label">Unique Customers</div><div class="progga-stat-value">{{ $uniqueCustomers }}</div></div></div></div></div>
  <div class="progga-card"><div class="progga-card-header"><div class="progga-card-title">Sales &amp; Order Summary</div><span class="progga-badge progga-badge-secondary report-period-badge"><i class="bi bi-calendar3"></i> {{ $startDate->format('d M, Y') }} - {{ $endDate->format('d M, Y') }}</span></div><div class="progga-table-wrapper" style="border:none;border-radius:0;"><table class="progga-table"><thead><tr><th>Period</th><th>Total Sale</th><th>Honored</th><th>Product Discount</th><th>Total Order</th></tr></thead><tbody>@forelse($periodRows as $row)<tr><td><strong>{{ $row['period'] }}</strong></td><td><strong>৳{{ number_format($row['total_sale'], 0) }}</strong></td><td><strong>৳{{ number_format($row['total_discount'] ?? 0, 0) }}</strong></td><td><strong>৳{{ number_format($row['total_product_discount'] ?? 0, 0) }}</strong></td><td>{{ number_format($row['total_order']) }}</td></tr>@empty<tr><td colspan="5" class="text-center py-4 text-muted">No sales/order data found.</td></tr>@endforelse</tbody><tfoot><tr><th>Total</th><th>৳{{ number_format($periodTotalSale, 0) }}</th><th>৳{{ number_format($periodTotalDiscount ?? collect($periodRows)->sum('total_discount'), 0) }}</th><th>৳{{ number_format($periodTotalProductDiscount ?? collect($periodRows)->sum('total_product_discount'), 0) }}</th><th>{{ number_format($periodTotalOrder) }}</th></tr></tfoot></table></div><div class="progga-card-footer">@include('admin.reports.partials.custom_pagination', ['paginator' => $periodRows])</div></div>
</main>
@endsection
@section('script')
