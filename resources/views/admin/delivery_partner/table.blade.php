...
<td><i class="bi bi-truck text-secondary me-2"></i>{{ $partner->name }}</td>
<td><span class="progga-badge">{{ $partner->status?'Active':'Inactive' }}</span></td>
<td>
<button class="progga-btn progga-btn-outline progga-btn-icon progga-btn-sm" title="Edit" onclick="editDeliveryPartner({{ $partner->id }})"><i class="bi bi-pencil"></i></button>
<button class="progga-btn progga-btn-danger progga-btn-icon progga-btn-sm" title="Delete" onclick="deleteDeliveryPartner({{ $partner->id }})"><i class="bi bi-trash"></i></button>
</td>
</tr>
@empty
<tr><td colspan="4" class="text-center">No delivery partner found</td></tr>
@endforelse
</tbody>
</table>
</div>
<div class="progga-card-footer" style="display:flex;align-items:center;justify-content:space-between;padding:15px;">
<span class="progga-page-info" style="font-size:13px;color:var(--progga-text-muted);">Showing {{ $partners->firstItem() ?? 0 }}–{{ $partners->lastItem() ?? 0 }} of {{ $partners->total() }}</span>
@if($partners->hasPages())
<div class="progga-pagination custom-pagination-wrapper">
@if($partners->onFirstPage())<span class="disabled"><i class="bi bi-chevron-left"></i></span>@else<a href="{{ $partners->previousPageUrl() }}"><i class="bi bi-chevron-left"></i></a>@endif
@foreach($partners->getUrlRange(max(1,$partners->currentPage()-2),min($partners->lastPage(),$partners->currentPage()+2)) as $page=>$url)
<a class="{{ $page==$partners->currentPage()?'active':'' }}" href="{{ $url }}">{{ $page }}</a>
@endforeach
@if($partners->hasMorePages())<a href="{{ $partners->nextPageUrl() }}"><i class="bi bi-chevron-right"></i></a>@else<span class="disabled"><i class="bi bi-chevron-right"></i></span>@endif
</div>
@endif
</div>
